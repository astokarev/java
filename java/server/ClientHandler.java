        ...
        while (true) {
            ...
            switch (tokens[0]) {
                ...
                case "/w": {// /w <nick> <message>
                    ...
                    StringBuilder msg = new StringBuilder(tokens[2] + " ");
                    int i = 3;
                    while (i < tokens.length) {
                        //msg = new StringBuilder(tokens[i]);
                        msg.append(tokens[i]).append(" ");
                        i++;
                    }

                    String msg_final = "(Шепчет) " + msg;
                    myServer.sendMsgToClient(this, nick, msg_final);
                    break;
                }
                case "/coin": {
                    Random rnd = new Random();
                    Message msg = new Message();
                    msg.setMessage(nick + " подбрасывает монетку");
                    myServer.broadcastMessage(msg);
                    boolean flip = rnd.nextBoolean();
                    msg.setMessage(flip ? "Выпала решка" : "Выпал орёл");
                    myServer.broadcastMessage(msg);
                }
            }
        }
